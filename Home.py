...
* **Sector News Signal** - provides a long / short signal generated by an AI / machine learning model trained on an historical news corpus, with an associated confidence score.
* **Sector Analysis** - enables the research of correlated and co-integrated pairs, plotting the price performance, selects top correlations and provides a correlation matrix.
* **Sector Backtesting** - can simulate a back test using the clasical statistical arbitrage methodology, using the z-score parameter as the entry signal, and profit target, stop loss or Z-score as the exit signal.
* **Crypto Analysis** - analysis of crypto assets from selected exchanges.
* **Crypto Backtesting** - simulation of crypto assets from selected exchanges.
